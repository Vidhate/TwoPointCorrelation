...
vector<double> countsBruteForce(vector<vector<double> > positions, vector<vector<double> > positionsRR, 
	...
	vector<vector<double> > adjacencyDD;
	vector<vector<double> > adjacencyRR;
	vector<double> dummy(n,-1.0);
	for(long long i=0;i<n;i++){
		adjacencyDD.push_back(dummy);
		adjacencyRR.push_back(dummy);
	}
	
	double sepDD,sepRR;
	for(long long i=0;i<n;i++){
		for(long long j=0;j<n;j++){
			if(adjacencyDD.at(i).at(j)<0.0){
				adjacencyDD.at(i).at(j)=separation(positions.at(i),positions.at(j));
				adjacencyDD.at(j).at(i)=adjacencyDD.at(i).at(j);
			}
			sepDD=adjacencyDD.at(i).at(j);

			if(adjacencyRR.at(i).at(j)<0.0){
				adjacencyRR.at(i).at(j)=separation(positionsRR.at(i),positionsRR.at(j));
				adjacencyRR.at(j).at(i)=adjacencyRR.at(i).at(j);
			}
			sepRR=adjacencyRR.at(i).at(j);
